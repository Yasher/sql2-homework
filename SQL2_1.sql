...
SELECT tab.store_id, COUNT(1)  FROM  (SELECT
	c.store_id,
	s.address_id,
	a.city_id,
	c2.city,
...
s2.store_id = s.store_id) tab 
GROUP BY tab.store_id ;
...
SELECT stf.first_name , stf.last_name  FROM sakila.staff stf WHERE stf.store_id = (SELECT
	cst.store_id
FROM
	sakila.customer cst
GROUP BY
...
FROM
	sakila.store str
	JOIN sakila.address a ON a.address_id = str.address_id
